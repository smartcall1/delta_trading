"""RiseX 거래소 어댑터 — Onchain CLOB, EIP-712 permit"""
...
class RiseXExchange(BaseExchange):
    name = "risex"
    ...
    def _ensure_client(self):
        if self._client is None:
            from exchanges._risex_raw import RiseXClient
            self._client = RiseXClient(**self._init_kwargs)
    ...
    async def cancel_all_orders(self, symbol: str) -> int:
        ...
        return 1 if not resp.get("error") else 0
    ...
    async def set_leverage(self, symbol: str, leverage: int) -> bool:
        try:
            ...
            logger.warning("RiseX set_leverage failed: %s", e)
            return False
