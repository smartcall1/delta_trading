...
| `hotstuff` | Hotstuff | REST + EIP-712 (msgpack) | private_key |
| `risex` | RiseX | REST + EIP-712 (permit) | private_key + account_address |

---

## 거래소별 수수료

| 거래소 | Maker Fee | Taker Fee | 비고 |
|--------|-----------|-----------|------|
| dango | 0.00% | 0.05% | maker 무료 |
| standx | 0.02% | 0.04% | |
| hibachi | 0.00% | 0.045% | maker 무료 |
| grvt | -0.01% | 0.03% | maker 리베이트 |
| hotstuff | 0.01% | 0.035% | |
| risex | 0.00% | 0.05% | maker 무료 |

페어 선택 시 **Maker fee가 낮은 거래소를 maker로**, Taker fee가 낮은 거래소를 taker로 배치하는 것이 유리.

---

## 멀티슬롯 운영

거래소 페어별로 별도 프로세스로 동시 운영 가능:

```bash
# 슬롯 1: Dango↔StandX (ETH)
python main.py --config slot_dango_standx.env

# 슬롯 2: Hotstuff↔RiseX (ETH)
python main.py --config slot_hotstuff_risex.env

# 슬롯 3: Dango↔Hibachi (BTC)
python main.py --config slot_dango_hibachi_btc.env
```

각 슬롯은 독립적 상태머신/로그/텔레그램 알림으로 동작. 텔레그램 메시지에 `📡 maker↔taker` 페어 레이블이 포함되어 어떤 슬롯인지 구분 가능.
...
```env
# 거래소 선택 (가능한 값: dango, standx, hibachi, grvt, hotstuff, risex)
...
| 거래소 | 필드 |
|--------|------|
| dango | `PRIVATE_KEY`, `ACCOUNT_ADDRESS` |
| standx | `JWT_TOKEN`, `PRIVATE_KEY` |
| hibachi | `API_KEY`, `PRIVATE_KEY`, `PUBLIC_KEY`, `ACCOUNT_ID` |
| grvt | `API_KEY`, `PRIVATE_KEY`, `ACCOUNT_ID` |
| hotstuff | `PRIVATE_KEY` |
| risex | `PRIVATE_KEY`, `ACCOUNT_ADDRESS` |
...
```env
# PAIR:거래소내부심볼 (콤마 구분)
SYMBOL_MAP_DANGO=ETH:perp/ethusd,BTC:perp/btcusd
SYMBOL_MAP_STANDX=ETH:ETH-USD,BTC:BTC-USD
SYMBOL_MAP_HOTSTUFF=ETH:ETH-PERP,BTC:BTC-PERP
SYMBOL_MAP_RISEX=ETH:ETH,BTC:BTC
```
...
VPS/서버:
```bash
nohup python -u main.py --config slot1.env > /dev/null 2>&1 &
```
...
모든 알림 메시지에 `📡 maker↔taker` 레이블이 포함되어 멀티슬롯 운영 시 구분 가능.
...
### 4. Config + .env + main.py

- `config.py`: `SYMBOL_MAP_NEWEX`, `TICK_SIZE_NEWEX` 필드 추가
- `main.py`: `_build_exchange_kwargs`에 인증 파라미터 매핑 추가
- `.env.example`: 심볼/틱사이즈 예시 추가
- `EXCHANGE_FEES`: config.py에 수수료 추가
...
│   ├── _hotstuff_raw.py    # Hotstuff raw 클라이언트 (EIP-712 + msgpack)
│   └── _risex_raw.py       # RiseX raw 클라이언트 (EIP-712 permit)
├── logs/                   # 런타임 로그 + 상태 파일
├── slot_*.env              # 슬롯별 환경설정
├── .env.example            # 설정 템플릿
...
| Maker | Taker | 설명 |
|-------|-------|------|
| dango | standx | Dango maker(무료) + StandX taker |
| dango | hibachi | Dango maker(무료) + Hibachi taker |
| dango | grvt | Dango maker(무료) + GRVT taker |
| grvt | standx | GRVT maker(리베이트) + StandX taker |
| hotstuff | risex | Hotstuff maker + RiseX taker |
| risex | hotstuff | RiseX maker(무료) + Hotstuff taker |
| dango | hotstuff | Dango maker(무료) + Hotstuff taker |

Maker는 **post-only fee가 낮은** 거래소가 유리하고, Taker는 **IOC/시장가가 빠르고 fee가 낮은** 거래소가 유리.
